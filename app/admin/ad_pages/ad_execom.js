...
export default function ExecomAdmin() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (selectedItem) {
                    setMembers(members.map(i => i.id === selectedItem.id ? item : i));
                } else {
    ...
    const handleDelete = async (index, id) => {
        if (id) {
            await fetch(`/api/admin?type=execom&id=${id}`, { method: 'DELETE' });
        }
